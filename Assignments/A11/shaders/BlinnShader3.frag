...
void main() {
	...
	vec3 diff = gubo.lightPos - fragPos;
    vec3 lightDir = normalize(diff);
    vec3 lightColor = pow(g / length(diff), beta) * gubo.lightColor.rgb * clamp((dot(lightDir, gubo.lightDir) - cosout) / (cosin-cosout), 0.0f, 1.0f);
